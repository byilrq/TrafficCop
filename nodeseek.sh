...
manual_fresh() {
    ...
    for ch in $TG_CHANNELS; do
        ...
        echo "$now [$ch] 最新消息已更新" >> "$LOG_FILE"
        if [[ ${#messages[@]} -gt 0 ]]; then
            echo "$now [$ch] 当前关键词：$KEYWORDS" >> "$LOG_FILE"
            echo "$now [$ch] 最新10条消息匹配情况如下：" >> "$LOG_FILE"
            ...
            for msg in "${messages[@]}"; do
                ...
                for kw in $KEYWORDS_LOWER; do
                    if [[ "$msg_lower" == *"$kw"* ]]; then
                        matched=1
                        echo "$now [$ch] 匹配 ✔：${msg}（关键词：$kw）" >> "$LOG_FILE"
                        break
                    fi
                done
                if [[ $matched -eq 0 ]]; then
                    echo "$now [$ch] 未匹配 ✖：${msg}" >> "$LOG_FILE"
                fi
            done
            echo "$now [$ch] ⚠️无匹配或均已推送过" >> "$LOG_FILE"
        fi
...
# ============================================
if [[ "$1" == "-cron" ]]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') 🚀 定时任务已启动（每20秒执行 manual_fresh + auto_push）" >> "$CRON_LOG"
    while true; do
        trim_file() {
            local file="$1"
            local max_lines=100
        ...
        trim_file "$WORK_DIR/sent_nodeseekc.txt"
        {
            ...
            echo "$(date '+%Y-%m-%d %H:%M:%S') 🕒 等待20秒进入下次周期..." >> "$CRON_LOG"
            echo "" >> "$CRON_LOG"
        } &
        wait
        sleep 20
